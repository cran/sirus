...
public:
  ...
  std::vector<std::vector<std::vector<double>>> getForestPaths() {
    std::vector<std::vector<std::vector<double>>> result;
    for (std::map<std::vector<std::vector<double>>, int>::iterator it = forest_paths.begin(); it != forest_paths.end(); ++it){
      result.push_back(it->first);
    }
    return result;
  }
  std::vector<int> getPathsProba() {
...
protected:
  ...
  // Path extraction
  std::map<std::vector<std::vector<double>>, int> forest_paths;
